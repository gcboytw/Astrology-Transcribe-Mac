...
try:
    ...
    glossary = ["相位", "合相", "三分相", "六分相", "四分相", "對相", "宮位"]
...
3. 絕對不可合併或刪除句子！我給你 {len(texts)} 行，你必須精準地回覆我 {len(texts)} 行。
4. 請維持我給你的「[編號]」格式。
5. 只輸出結果，不要說「好的」或給予任何解釋。
...
"""
    try:
        ...
        tqdm.write("\n⏳ [AI 思考中...]")
        # 降低 temperature 到 0.1，讓模型只專注修改錯字，不擅自發揮創意寫作
...
def process_file(file_path):
    ...
    srt_out = os.path.join(FINAL_OUTPUT_DIR, f"{filename}-ok.srt")
    txt_out = os.path.join(FINAL_OUTPUT_DIR, f"{filename}-ok.txt")
    ...
    for i in tqdm(range(0, len(segments), batch_size), desc="字幕段落處理進度"):
        ...
        if corrected_lines is None:
            ...
            corrected_lines = [correct_single_text(text) for text in texts]
    ...
    with open(md_out, 'w', encoding='utf-8') as f:
        ...
        for i, s in enumerate(corrected_segments):
            ...
            # 若已經累積滿 10 句，或是遇到明顯的句點、問號、驚嘆號結尾 (包含全形與半形)，就當作一個段落輸出
            clean_end = text.strip()
            if len(paragraph_text) >= 10 or clean_end.endswith(('。', '？', '！', '.', '?', '!')):
...
def main():
    print("✨ 啟動 AI 智能本地校正腳本 (由 Ollama 的 qwen2.5:7b 大神加持) ...")
